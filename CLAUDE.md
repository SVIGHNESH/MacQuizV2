...
- Same degrade-not-fail posture for optional integrations: email (Resend), telemetry (OTel -> Grafana Cloud), S3 blob storage for imports/avatars - each disabled when its env vars are unset (see `server/internal/config/config.go`, every knob has a dev default).
