...
// XLSX reads a .xlsx upload into records, header first, shaped exactly like
// CSV so callers validate both formats with one code path.
//
...
// graph; someone filling in a single-sheet template never has more than one
// sheet to read from, so that graph is unneeded complexity here.
func XLSX(r io.Reader) ([][]string, error) {
	...
	if len(records) == 0 {
		return nil, fmt.Errorf("read header: empty sheet")
	}
	return records, nil
}
...
// shape CSV works from, placing each cell at its real column index (from its
// "r" ref, e.g. "C7") rather than its position in the XML - spreadsheet
// software omits empty trailing cells from a row entirely, so positional
// indexing alone would silently shift later columns left.
