...
- Dashboard and alert-rule definitions are checked in as `scripts/grafana/{dashboard.json,alert-rules.json}` (see `scripts/grafana/README.md`): a Grafana dashboard for the four key series above, plus alert rules for the three section 3 thresholds backed by a metric in this Grafana Cloud stack (autosave p95, queue lag, and disk usage on the pg volume). Grafana Cloud's free tier has no filesystem provisioning, so these are imported once by hand via the UI or the alert-provisioning HTTP API, the same "run by hand" convention `restore-drill.sh` uses. Disk usage has no *application*-emitted metric behind it (it's host-level, not a request path `server/internal/telemetry` instruments), but is covered by the `alloy` container in `docker-compose.prod.yml` (`scripts/grafana/alloy-disk-metrics.alloy`): a host metrics agent that scrapes the `pg_data` volume's filesystem stats and forwards them to the same OTLP gateway the app/worker export to, so the alert rule can still query it from the same Prometheus/Mimir datasource. Backup-job alerting is covered separately by `backup.sh`'s dead-man's-switch ping (section 1 above), not by a Grafana alert rule.
